...
fruits <- ratings_trials_exp %>% 
  filter(concept == c("blueberry", "lemon", "mango", "strawberry", "watermelon", "apple", "banana", "cherry", "grape", "peach")) %>% 
...
veggies <- ratings_trials_exp %>% 
  filter(concept == c("carrot", "celery", "corn", "eggplant", "mushroom")) %>% 
  group_by(concept, texture) %>% 
  summarize(AverageRatings = mean(response), .groups = "drop")
...
weather <- ratings_trials_exp %>% 
  filter(concept == c("blizzard", "drought", "hurricane", "lightning", "sandstorm")) %>% 
  group_by(concept, texture) %>% 
  summarize(AverageRatings = mean(response), .groups = "drop")
...
clothes <- ratings_trials_exp %>% 
  filter(concept == c("dress", "pants", "shirt", "socks", "shoes")) %>% 
  group_by(concept, texture) %>% 
  summarize(AverageRatings = mean(response), .groups = "drop")
...
scenes <- ratings_trials_exp %>% 
  filter(concept == c("beach", "field", "ocean", "sky", "sunset")) %>% 
  group_by(concept, texture) %>% 
  summarize(AverageRatings = mean(response), .groups = "drop")
...
emotions <- ratings_trials_exp %>% 
  filter(concept == c("angry", "disgust", "fearful", "happy", "sad")) %>% 
  group_by(concept, texture) %>% 
  summarize(AverageRatings = mean(response), .groups = "drop")
...
values <- ratings_trials_exp %>% 
  filter(concept == c("evil", "greed", "justice", "love", "peace")) %>% 
  group_by(concept, texture) %>% 
  summarize(AverageRatings = mean(response), .groups = "drop")
...
```{r}
#carquals
carquals <- ratings_trials_exp %>% 
  filter(concept == c("comfort", "efficiency", "speed", "safety", "reliability")) %>% 
  group_by(concept, texture) %>% 
  summarize(AverageRatings = mean(response), .groups = "drop")

# Generate all concept pairs
carquals_concepts <- unique(carquals$concept)
carquals_concept_pairs <- combn(carquals_concepts, 2, simplify = FALSE) 

carquals_reshaped <- carquals %>%
  spread(key = concept, value = AverageRatings)
...
# Loop through each concept pair
for (pair in carquals_concept_pairs){
  # Loop through each combination of two textures
  for (textures in texture_pairs) {
    texture1 <- as.character(textures[[1]])
    texture2 <- as.character(textures[[2]])
    ...
    if (texture1 != texture2) {
      # Extract ratings
    concept1_texture1_rating <- as.numeric(carquals_reshaped[carquals_reshaped$texture == texture1, pair[1]]) # X1
    concept1_texture2_rating <- as.numeric(carquals_reshaped[carquals_reshaped$texture == texture2, pair[2]]) # X4
    concept2_texture1_rating <- as.numeric(carquals_reshaped[carquals_reshaped$texture == texture1, pair[2]]) # X2
    concept2_texture2_rating <- as.numeric(carquals_reshaped[carquals_reshaped$texture == texture2, pair[1]]) # X3
...
# Add identifier column with all the info
semantic_distances_carquals_df <- semantic_distances_df %>%
  mutate(identifier = paste(concept1, concept2, texture1, texture2, sep = "_"))

#add group name column
semantic_distances_carquals_df <- semantic_distances_carquals_df %>% 
  mutate(
    group = "carquals"
  )
...
location <- ratings_trials_exp %>% 
  filter(concept == c("above", "below", "beside", "near", "far")) %>% 
  group_by(concept, texture) %>% 
  summarize(AverageRatings = mean(response), .groups = "drop")
...
time <- ratings_trials_exp %>% 
  filter(concept == c("dawn", "day", "dusk", "night", "noon")) %>% 
  group_by(concept, texture) %>% 
  summarize(AverageRatings = mean(response), .groups = "drop")
...
transport <- ratings_trials_exp %>% 
  filter(concept == c("airplane", "car", "boat", "truck", "train")) %>% 
  group_by(concept, texture) %>% 
  summarize(AverageRatings = mean(response), .groups = "drop")
...
activity <- ratings_trials_exp %>% 
  filter(concept == c("driving", "eating", "sleeping", "leisure", "working")) %>% 
  group_by(concept, texture) %>% 
  summarize(AverageRatings = mean(response), .groups = "drop")
...
semantic_distances_carquals_df <- merge(semantic_distances_carquals_df, percept_distances_df, by = c("texture1", "texture2"), all.x = FALSE, all.y = TRUE)
...
write.csv(semantic_distances_carquals_df, "semantic_distances/carquals.csv", row.names = FALSE)
